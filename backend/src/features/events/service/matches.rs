...
use crate::{
    app::state::AppState,
    features::{
        events::models::{CreateEventMatchInput, Event, Match, ReportMatchWinnerInput, SetMatchupInput},
...
pub async fn generate_tourney_bracket_for_user(
    ...
    matches_service::generate_tourney_bracket_for_user(state, user_id, event_id).await
}
